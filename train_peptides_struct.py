...
parser.add_argument("--dim_v", default=96, type=int)
...
parser.add_argument("--name", default="peptides-struct", type=str)
...
def train_step(state, batch):
    ...
    def loss_fn(params):
        ...
        loss = jnp.abs(logits - batch["y"]).mean()
        return loss
...
def eval_step(state, batch):
    ...
    eval_loss = jnp.abs(logits - batch["y"]).mean()
